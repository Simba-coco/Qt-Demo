    ...
    else if(new_System==10)
    {
        ...
        if(old_System==2||old_System==8||old_System==16)
        {
            ...
            if(pointPosition==-1)
            {
                ...
                {
                    if(num[length-i-1]!='0')
                      dl=dl+toNumber(num[length-i-1])*pow((double)old_System,i*1.0);
                }
            }
            else
            {
                for(int i=0;i<pointPosition;i++)
                {
                    if(num[pointPosition-i-1]!='0')
                        dl=dl+toNumber(num[pointPosition-i-1])*pow((double)old_System,i);
                }
                for(int i=pointPosition+1;i<length;i++)
                {
                    if(num[i]!='0')
                    {dl=dl+toNumber(num[i])/pow((double)old_System,(double)i-pointPosition);}
                }
            }
