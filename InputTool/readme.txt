应该clone自刘大神的，很长时间了，仅供有需要的参考学习，同时谢谢大神的奉献。
...
13：用法简单极致，只需要在项目中添加输入法界面类（共三个文件frminput.h/frminput.cpp/frminput.ui），
然后在main函数中引入头文件#include "frminput.h"后调用frmInput::Instance()->hide();即可。整个工程都可以用。
